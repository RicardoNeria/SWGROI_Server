...
-- TABLA TICKETS - VERSIÓN COMPLETA CON TIPO_ASUNTO Y FIX DE FECHAACTUALIZACION
...
-- -----------------------
-- FIX FECHAACTUALIZACION: ARMONIZACIÓN Y BACKFILL
-- -----------------------

-- 1) Si existe la columna PascalCase, eliminarla de forma segura
SET @drop_pascal := (
  SELECT IF(
    EXISTS (
      SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_SCHEMA='swgroi_db' AND TABLE_NAME='tickets' AND COLUMN_NAME='FechaActualizacion'
    ),
    'ALTER TABLE tickets DROP COLUMN FechaActualizacion',
    'SELECT 1'
  )
);
PREPARE stmt FROM @drop_pascal; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- 2) Si existe la columna snake_case, renombrarla a PascalCase; si no existe, crearla
SET @rename_or_add := (
  SELECT CASE
    WHEN EXISTS (
      SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_SCHEMA='swgroi_db' AND TABLE_NAME='tickets' AND COLUMN_NAME='fecha_actualizacion'
    ) THEN 
      'ALTER TABLE tickets CHANGE COLUMN fecha_actualizacion FechaActualizacion DATETIME NULL'
    ELSE 
      'ALTER TABLE tickets ADD COLUMN FechaActualizacion DATETIME NULL'
  END
);
PREPARE stmt FROM @rename_or_add; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- 3) Backfill: si está NULL, usar FechaRegistro (compatible con Safe Update Mode)
-- Guardar y desactivar modo seguro para permitir UPDATE
SET @prev_safe_updates := @@SQL_SAFE_UPDATES;
SET SQL_SAFE_UPDATES = 0;

-- Método robusto usando JOIN sobre PK para cumplir reglas de safe-update
UPDATE tickets t
JOIN (
  SELECT id FROM tickets WHERE FechaActualizacion IS NULL
) s ON s.id = t.id
SET t.FechaActualizacion = t.FechaRegistro;

-- Restaurar configuración original de safe updates
SET SQL_SAFE_UPDATES = @prev_safe_updates;

-- 4) Crear trigger para auto-actualizar FechaActualizacion en UPDATE
DROP TRIGGER IF EXISTS tr_tickets_fechaactualizacion;
DELIMITER $$
CREATE TRIGGER tr_tickets_fechaactualizacion 
...
-- Verificar columna FechaActualizacion
SELECT 
    'COLUMNA FECHAACTUALIZACION' AS verificacion,
    CASE WHEN COUNT(*) > 0 THEN 'OK' ELSE 'ERROR' END AS estado
FROM INFORMATION_SCHEMA.COLUMNS 
WHERE TABLE_SCHEMA = DATABASE() 
  AND TABLE_NAME = 'tickets' 
...
-- Verificar datos de FechaActualizacion
SELECT 
    COUNT(*) as total_tickets,
    COUNT(FechaActualizacion) as tickets_con_fecha_actualizacion,
    COUNT(FechaRegistro) as tickets_con_fecha_registro,
    ROUND((COUNT(FechaActualizacion) / COUNT(*)) * 100, 2) as porcentaje_completado
FROM tickets;
...
- FIX COMPLETO de FechaActualizacion (armonización y backfill automático)
- Trigger automático para actualizar FechaActualizacion en UPDATE
