            ...
            string ip, string userAgent)
        {
            return Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        cn.Open();
                        // Garantizar que la tabla exista en instalaciones nuevas
                        try { DB.SchemaInitializer.EnsureAuditoriaTable(cn); } catch { /* best-effort */ }
                ...
                catch (Exception ex)
                {
                    ...
                    try { Console.WriteLine($"[AuditLogger] {ex.Message}"); } catch {}
                }
            });
        }
