...
// DEBUG flag y utilidades comunes
const DEBUG = false;
const logDbg = (...args) => { if (DEBUG) { try { console.log('[tickets]', ...args); } catch {} } };

// Límites de UI centralizados
const UI_LIMITS = Object.freeze({
    DESC_MAX: 80,
    COMENT_MAX: 60,
    DESC_CONTADOR_MAX: 2000 // contador visual
});

// Utilidades de texto/escape
const TextUtils = {
    toStringSafe(v) { return (v == null ? '' : String(v)); },
    escapeHtml(v) {
        const s = this.toStringSafe(v);
        return s
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;');
    ...
    csvEscape(v) {
        const s = this.toStringSafe(v).replace(/"/g, '""');
        return `"${s}"`;
    }
};

// Conversión de Tipo de Asunto (UI <-> Backend)
const TipoAsunto = {
    toShort(v) {
        const t = (v || '').trim();
        if (t === 'Mant. Correctivo Panel') return 'Mant. Correctivo';
        if (t === 'Mant. Preventivo Panel') return 'Mant. Preventivo';
        return t;
    },
    toBackend(v) {
        const t = (v || '').trim();
        if (t === 'Mant. Correctivo') return 'Mant. Correctivo Panel';
        if (t === 'Mant. Preventivo') return 'Mant. Preventivo Panel';
        return t;
    }
};

// (Simplificado) Los values del select ya coinciden con lo que espera el backend (se convierte al enviar y al mostrar en el form).
...
const TicketOperations = {
    ...
    async registrarTicket(datos) {
        try {
            // Validación previa de folio duplicado (opcional - mejora UX)
            if (await this.validarFolioDuplicado(datos.Folio)) {
                throw new Error('El folio del ticket de servicio ya existe, favor de generar uno nuevo.');
            }
    ...
    async validarFolioDuplicado(folio) {
        if (!folio || !folio.trim()) return false;
        
        try {
            // Buscar en la lista actual de tickets cargados
            const folioExistente = TicketsModule.tickets.find(ticket => 
                (ticket.Folio || '').toLowerCase() === folio.toLowerCase()
            );
            
            return !!folioExistente;
        } catch (error) {
            console.warn('Error al validar folio duplicado:', error);
            return false; // En caso de error, permitir que el backend maneje la validación
        }
    },
    ...
    async cargarTickets() {
        try {
            ...
            // DEBUG controlado
            logDbg('🔍 Tickets cargados desde servidor:', TicketsModule.tickets);
            if (TicketsModule.tickets.length > 0) {
                logDbg('🎯 Primer ticket TipoAsunto:', TicketsModule.tickets[0].TipoAsunto);
            }
...
const TicketsUIUpdater = {
    ...
    // Crear celdas con funcionalidad "Leer más" SIEMPRE en Descripción
    const descripcionTruncada = this.truncarTexto(ticket.Descripcion || '', UI_LIMITS.DESC_MAX);
    const comentarioTruncado = this.truncarTexto(ticket.Comentario || '', UI_LIMITS.COMENT_MAX);
        
    // Preparar identificador para el control accesible de descripción
    const idDesc = `desc-${(ticket.Folio || '').toString().replace(/[^a-zA-Z0-9_-]/g, '')}`;
    const descripcionClass = 'ui-tabla__cell';
        
        // DEBUG: Ver TODOS los campos que llegan para este ticket
        logDbg(`🎯 Ticket ${ticket.Folio}:`, {
            TipoAsunto: ticket.TipoAsunto,
            tipo_asunto: ticket.tipo_asunto,
            AllFields: Object.keys(ticket),
            FullTicket: ticket
        });
        
    // Tomar TipoAsunto desde JSON, compatible con ambas variantes
    const tipoAsuntoValue = TipoAsunto.toShort((ticket.TipoAsunto || ticket.tipo_asunto || '').toString());
        ...
        tr.innerHTML = `
            ...
            <td class="${descripcionClass}">
                <div class="tickets-desc" data-role="descripcion">
                    <span id="${idDesc}" class="tickets-desc__texto" data-full-text="${TextUtils.escapeAttr(ticket.Descripcion || '')}" data-collapsed="true">${TextUtils.escapeHtml(descripcionTruncada)}</span>
                    ${(ticket.Descripcion || '').length > UI_LIMITS.DESC_MAX ? `<button type="button" class="ui-link ui-link--toggle" data-action="toggle-desc" data-target="${idDesc}" aria-expanded="false" aria-controls="${idDesc}">Ver más</button>` : ''}
                </div>
            </td>
...
const TicketsFormManager = {
    ...
    llenarFormulario(ticket) {
        ...
        if (elements.tipoAsunto) {
            const ta = (ticket.TipoAsunto || '').trim();
            // Mostrar sin el sufijo "Panel" para estas dos opciones
            elements.tipoAsunto.value = TipoAsunto.toShort(ta);
        }
    ...
    actualizarContador() {
        ...
        if (descripcion && contador) {
            ...
            contador.textContent = `${length}/${UI_LIMITS.DESC_CONTADOR_MAX}`;
        }
    },
    
    validarFormulario() {
        const folio = document.getElementById('folio')?.value.trim();
        const tipoAsunto = document.getElementById('tipoAsunto')?.value.trim();
...
const TicketsExportManager = {
    exportarCSV() {
        ...
        const headers = ['Folio', 'Tipo Asunto', 'Descripción', 'Estado', 'Responsable', 'Comentario'];
        
        const csvContent = [
            headers.join(','),
            ...ticketsFiltrados.map(ticket => [
                TextUtils.csvEscape(ticket.Folio || ''),
                TextUtils.csvEscape(TipoAsunto.toShort(ticket.TipoAsunto || '')),
                TextUtils.csvEscape(ticket.Descripcion || ''),
                TextUtils.csvEscape(TicketsUIUpdater.formatearEstado(ticket.Estado || '')),
                TextUtils.csvEscape(ticket.Responsable || ''),
                TextUtils.csvEscape(ticket.Comentario || '')
            ].join(','))
        ].join('\n');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            // Convertir a formato backend si corresponde
            const tipoAsuntoUI = document.getElementById('tipoAsunto')?.value.trim();
            const tipoAsuntoBackend = TipoAsunto.toBackend(tipoAsuntoUI);

            const datos = {
                Folio: document.getElementById('folio')?.value.trim(),
                TipoAsunto: tipoAsuntoBackend,
    ...
    // Delegación de eventos para la tabla - UN SOLO LISTENER (acciones y Leer más)
    const tbody = document.querySelector('#tablaTickets tbody');
    if (tbody) {
        tbody.addEventListener('click', function(e) {
            // PRIMERO: Verificar si es un botón de acción
            const button = e.target.closest('button[data-action]');
            if (button) {
                // Detener completamente la propagación del evento para botones
                e.stopPropagation();
                ...
                switch (action) {
                    ...
                    case 'delete':
                        // Reestablecer modal personalizado de confirmación
                    ...
                    case 'toggle-desc': {
                        ...
                        if (!span) break;
                        const full = span.getAttribute('data-full-text') || '';
                        const collapsed = span.getAttribute('data-collapsed') !== 'false';
                        if (collapsed) {
                            // Expandir
                            ...
                            // Colapsar
                            span.textContent = TicketsUIUpdater.truncarTexto(full, UI_LIMITS.DESC_MAX);
                ...
                return; // Salir sin procesar "Leer más"
            }

            // SEGUNDO: si se hace clic en el texto de la descripción, expandir/colapsar inline
            const textoDesc = e.target.closest('.tickets-desc__texto');
            if (textoDesc) {
                const full = textoDesc.getAttribute('data-full-text') || '';
                const collapsed = textoDesc.getAttribute('data-collapsed') !== 'false';
                if (collapsed) {
                    textoDesc.textContent = full;
                    textoDesc.setAttribute('data-collapsed', 'false');
                } else {
                    textoDesc.textContent = TicketsUIUpdater.truncarTexto(full, UI_LIMITS.DESC_MAX);
                    textoDesc.setAttribute('data-collapsed', 'true');
                }
            }
        });
    }

    // Ya no se usa modal "Leer más"; la descripción es expandible inline.
...
// Sin modal de descripción: sólo expandir/colapsar inline
