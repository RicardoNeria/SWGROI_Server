...
const DEBUG = false;
const logDbg = (...args) => { if (DEBUG) { try { console.log('[tecnicos]', ...args); } catch {} } };
...
const TecnicosUIUpdater = {
    ...
    const fechaParaMostrar = ticket.FechaActualizacion || ticket.Fecha_Actualizacion || ticket.fecha_actualizacion || ticket.FechaRegistro || ticket.fecha_registro || '';
        ...
        tr.innerHTML = `
            ...
            <td class="ui-tabla__cell" title="${TextUtils.escapeAttr(ticket.Descripcion || '')}">${TextUtils.escapeHtml(descripcionTruncada)}</td>
            <td class="ui-tabla__cell">
                <span class="ui-badge ui-badge--${this.obtenerClaseEstado(ticket.Estado || '')}">
                    ${TextUtils.escapeHtml(ticket.Estado || '')}
                </span>
            </td>
    ...
    seleccionarTicket(folio) {
        ...
        if (ticket) {
            ...
            // Actualizar tipo de asunto visual
            const tipoAsuntoVisual = document.getElementById('tipoAsuntoVisual');
            if (tipoAsuntoVisual) {
                const tipoAsuntoValue = TipoAsunto.toShort((ticket.TipoAsunto || ticket.tipo_asunto || '').toString());
                tipoAsuntoVisual.textContent = tipoAsuntoValue || '--';
                tipoAsuntoVisual.classList.add('tecnicos-folio--selected');
            }
    ...
    limpiarFormulario() {
        ...
        if (tipoAsuntoVisual) {
            ...
            tipoAsuntoVisual.classList.remove('tecnicos-folio--selected');
        }
