...
const UserOperations = {
    async cargarUsuarios(mostrarMensaje = false) {
        try {
            ...
            // Solo mostrar mensaje cuando se solicite explícitamente (refrescos manuales)
            if (mostrarMensaje) {
                NotificationManager.show('Usuarios actualizados correctamente', 'success');
            }
        } catch (error) {
    ...
    async crearUsuario(datos) {
        try {
            ...
            NotificationManager.show('El usuario ha sido creado exitosamente', 'success');
    ...
    async actualizarUsuario(datos) {
        try {
            ...
            NotificationManager.show('El usuario ha sido actualizado exitosamente', 'success');
            ModalManager.hide('modalFormulario');
            await this.cargarUsuarios();
        } catch (error) {
    ...
    async eliminarUsuario(id) {
        try {
            ...
            NotificationManager.show('El usuario ha sido eliminado exitosamente', 'success');
...
const FormManager = {
    ...
    validarFormulario(datos) {
        ...
        if (!datos.Usuario || datos.Usuario.trim().length === 0) {
            ...
            // Validación de duplicidad local (antes de enviar al servidor)
            const usuarioExistente = AdminModule.usuarios.find(u => 
                u.Usuario.toLowerCase() === datos.Usuario.toLowerCase() && 
                (!AdminModule.usuarioEditando || u.IdUsuario !== AdminModule.usuarioEditando.IdUsuario)
            );
            if (usuarioExistente) {
                errores.usuario = 'Ya existe un usuario con este nombre. Por favor, elija otro nombre de usuario.';
            }
        }
...
const EventManager = {
    ...
    setupActionEvents() {
        ...
        if (btnRefrescar) {
            btnRefrescar.addEventListener('click', () => UserOperations.cargarUsuarios(true));
        }
...
// Auto-refresh para Admin/Usuarios: recarga cada 30s (sin mensaje)
(function(){
    if (typeof UserOperations === 'undefined') return;
    if (UserOperations._autoRefresh) return;
    UserOperations._autoRefresh = true;
    try { setInterval(() => { UserOperations.cargarUsuarios(false).catch(()=>{}); }, 30000); } catch(e) { console.warn('No se pudo iniciar auto-refresh admin', e); }
})();
