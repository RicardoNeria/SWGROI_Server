...
(function (window) {
  ...
  async upload() {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        util.toast(isNewVersion ? 'Versión creada' : 'Subida finalizada', 'success');
        try { if (window.SWGROI && window.SWGROI.UI && cont) window.SWGROI.UI.mostrarMensaje(isNewVersion ? 'Versión creada' : 'Documento subido', 'success', 'leyenda'); } catch(_) {}

        // mostrar feedback con nombre subido por un breve periodo
        try {
          const fb = document.getElementById('archivoFeedback');
          if (fb) {
            const name = (respJson && (respJson.nombre || respJson.filename || respJson.nombre_archivo)) || (fileInput?.files?.[0]?.name) || '';
        ...
        util.toast('Error subiendo archivo', 'error');
        try { if (window.SWGROI && window.SWGROI.UI && cont) window.SWGROI.UI.mostrarMensaje('Error subiendo archivo', 'error', 'leyenda'); } catch(_) {}
        try { const fb = document.getElementById('archivoFeedback'); if (fb) { fb.classList.remove('ok'); fb.classList.add('error'); fb.textContent = (respJson && respJson.mensaje) || 'Error subiendo archivo'; } } catch(_) {}
      }
    ...
    xhr.onerror = () => {
      util.toast('Error de red durante la subida', 'error');
      try { const cont = document.getElementById('leyenda'); if (window.SWGROI && window.SWGROI.UI && cont) window.SWGROI.UI.mostrarMensaje('Error de red durante la subida', 'error', 'leyenda'); } catch(_) {}
    };
  ...
  async download(id) {
    ...
    try {
      ...
      if (!r.ok) {
        const txt = await r.text().catch(() => null);
        util.toast('Error descargando archivo: ' + (txt || ('HTTP ' + r.status)), 'error');
        return;
      }
      ...
      if (window.showSaveFilePicker) {
        try {
          ...
          util.toast('Archivo guardado', 'success');
          return;
        } catch (e) {
      ...
      util.toast('Error descargando archivo', 'error');
    }
    finally {
  ...
  async update(formData) { try { const r = await fetch(this.urls.actualizar, { method: 'POST', body: formData }); const j = await r.json(); if (j && j.status === 'success') { try { await this.load(true); } catch(_) {} const msg = 'Documento actualizado'; util.toast(msg, 'success'); try { if (window.SWGROI && window.SWGROI.UI) window.SWGROI.UI.mostrarMensaje(msg, 'success', 'leyenda'); } catch(_) {} util.qs('#modalDetalles').style.display = 'none'; } else { util.toast('Error actualizando documento', 'error'); try { if (window.SWGROI && window.SWGROI.UI) window.SWGROI.UI.mostrarMensaje(j?.mensaje || 'Error actualizando documento', 'error', 'leyenda'); } catch(_) {} } } catch (e) { console.error(e); util.toast('Error actualizando', 'error'); try { if (window.SWGROI && window.SWGROI.UI) window.SWGROI.UI.mostrarMensaje('Error actualizando', 'error', 'leyenda'); } catch(_) {} } }
  ...
  async confirmDelete() {
    const id = this._toDeleteId; if (!id) return; try {
      ...
      const j = await r.json(); if (j && j.status === 'success') {
          try { await this.load(true); } catch(_) {}
          const msg = 'Documento eliminado';
          util.toast(msg, 'success');
          try { if (window.SWGROI && window.SWGROI.UI) window.SWGROI.UI.mostrarMensaje(msg, 'success', 'leyenda'); } catch(_) {}
        } else { const msg = j?.mensaje || 'No se pudo eliminar'; util.toast(msg, 'error'); try { if (window.SWGROI && window.SWGROI.UI) window.SWGROI.UI.mostrarMensaje(msg, 'error', 'leyenda'); } catch(_) {} }
  ...
  async toggleFavorito(id, btnEl) {
    try {
      ...
      if (btnEl) { prevActive = btnEl.classList.contains('btn-favorito--active'); btnEl.classList.toggle('btn-favorito--active'); btnEl.setAttribute('aria-pressed', String(!prevActive)); btnEl.title = !prevActive ? 'Quitar de favoritos' : 'Agregar a favoritos'; }
      const r = await fetch(this.urls.favorito + '&id=' + encodeURIComponent(id), { method: 'POST' });
      const j = await r.json();
      if (j && j.status === 'success') {
        // Preferir refrescar KPIs/lista en segundo plano SIN mostrar 'Buscando...'
        try { await this.load(true); } catch(_) {}
        // Preferir mensajes explícitos y consistentes en la "leyenda"
        const servidorFav = typeof j.favorito !== 'undefined' ? !!j.favorito : !prevActive;
        const mensaje = servidorFav ? 'Documento agregado a favoritos' : 'Documento eliminado de favoritos';
        // mostrar en leyenda (si existe) y también con toast
        try { if (window.SWGROI && window.SWGROI.UI) window.SWGROI.UI.mostrarMensaje(mensaje, 'success', 'leyenda'); else util.toast(mensaje, 'success'); } catch(_) { util.toast(mensaje, 'success'); }
        ...
        // revertir UI
  if (btnEl) { if (prevActive) btnEl.classList.add('btn-favorito--active'); else btnEl.classList.remove('btn-favorito--active'); btnEl.setAttribute('aria-pressed', String(prevActive)); btnEl.setAttribute('aria-label', prevActive ? 'Quitar de favoritos' : 'Agregar a favoritos'); btnEl.title = prevActive ? 'Quitar de favoritos' : 'Agregar a favoritos'; }
        util.toast(j?.mensaje || 'No se pudo actualizar favorito', 'error');
      }
    } catch (e) { console.error(e); util.toast('Error marcando favorito', 'error'); }
  }
